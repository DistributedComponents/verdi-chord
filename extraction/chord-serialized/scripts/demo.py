...
CHORD_SERIALIZED = os.path.join(os.path.dirname(__file__), "../chordserialized.native")

# Must agree with SUCC_LIST_LEN in ExtractChordSerialized.v
SUCC_LIST_LEN = 3
...
class Addr(object):
    ...
    # should agree with hash in ExtractChordSerialized.v
    def chordhash(self):
        md5.new(self.host).digest()

# Each node's "read_thread" executes this function to put logged information
# from its chordserialized.native process into the node's queue.
...
class Node(object):
    ...
    def spawn(self):
        args = [CHORD_SERIALIZED] + self.args
