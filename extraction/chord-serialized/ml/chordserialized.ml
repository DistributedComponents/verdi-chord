...
let mk_chord_config opts =
  ...
  then invalid_arg "keepalive timeout must be greater than request timeout"
    ...
    { ChordSerializedArrangement.tick_timeout = !(opts.tick_timeout)
    ; ChordSerializedArrangement.keepalive_timeout = !(opts.keepalive_timeout)
    ; ChordSerializedArrangement.request_timeout = !(opts.request_timeout)
    ; ChordSerializedArrangement.debug = !(opts.debug)
    }
...
let _ =
  ...
  ChordSerializedArrangement.run cc nm knowns
